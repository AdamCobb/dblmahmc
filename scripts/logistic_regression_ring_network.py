...
parser.add_argument('--num_samples', type=int, default=10000) #100
...
D[[1,2,3],[1,2,3]] = 2.
A = torch.zeros(5,5)
A[[0,1,1,2,2,3,3,4],[1,0,2,1,3,2,4,3]] = 1
L = D - A
print('Laplacian:')
print(L)
W = torch.inverse(torch.eye(5) + L)
...
if args.inference == 'dMALA' or 'DULA':
    ...
    for a in range(N_agents):
        X1 = []
        Y1 = []
        for i in range(a*digit_type_per_agent, a*digit_type_per_agent + digit_type_per_agent):
            X1.append(X[indices[i]][:limit])
            Y1.append(Y[indices[i]][:limit])
        X_list.append(torch.cat(X1))
        Y_list.append(torch.cat(Y1))
    ...
    for Y_local, X_local in zip(Y_list, X_list):
        ...
        Agent_log_prob_func_list.append(define_local_log_posterior(X_local.to(device), Y_local.view(-1,1).to(device), prec_agent = 10., n_agents=N_agents))
...
torch.save(config_file, '../exp/MNIST_ring_config_{:}_{:}_'.format(args.seed,args.step_size)  + args.inference + args.save + '.npy')
